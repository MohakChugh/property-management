...
const addproperty = async (type, sell_rent, price, partner_name, owner_name, locality, isFurnished, floor, fh_lh, contact_number, block, bhk_type, area, address, propertyCategory, unitNumber, description) => {
    ...
    const query = `mutation MyMutation {
        ...
        contact_number: "${contact_number}", description: "${description}", fh_lh: "${fh_lh}", floor: "${floor}", isFurnished: "", locality: "", owner_name: "", partner_name: "", price: "", propertyCategory: "", sell_rent: "", type: "", unitNumber: ""}) {
          affected_rows
          returning {
            id
          }
