...
const addproperty = async (type, sell_rent, price, partner_name, owner_name, locality, isFurnished, floor, fh_lh, contact_number, block, bhk_type, area, address) => {
    ...
    const query = `mutation MyMutation {
        __typename
        insert_propertydata(objects: {type: "${type}", sell_rent: "${sell_rent}", price: "", partner_name: "", owner_name: "", locality: "", isFurnished: "", floor: "", fh_lh: "", contact_number: "", block: "", bhk_type: "", area: "", address: ""}) {
          affected_rows
          returning {
            id
          }
