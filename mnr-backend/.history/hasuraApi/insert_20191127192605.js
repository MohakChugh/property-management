...
const addproperty = async (type, sell_rent, price, partner_name, owner_name, locality, isFurnished, floor, fh_lh, contact_number, block, bhk_type, area, address, propertyCategory, unitNumber, description) => {
    ...
    const query = `mutation MyMutation {
        __typename
        insert_propertydata(objects: {address: "${address}", area: "${area}", bhk_type: "${bhk}", block: "", contact_number: "", description: "", fh_lh: "", floor: "", isFurnished: "", locality: "", owner_name: "", partner_name: "", price: "", propertyCategory: "", sell_rent: "", type: "", unitNumber: ""}) {
          affected_rows
          returning {
            id
          }
